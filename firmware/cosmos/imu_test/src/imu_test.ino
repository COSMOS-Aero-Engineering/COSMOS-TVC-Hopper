...
// 생성자가 CS/WAK/INT/RST 4개 인자를 요구함(SPI용) — 기본 생성자가 없어서 인자 없이
// 쓸 수 없다. 우리는 I2C(.begin())만 쓰므로 이 4개는 실제로 안 쓰인다(생성자 몸체가
// 멤버변수에 저장만 하고, begin()의 I2C 경로는 이 값들을 안 건드림 — BNO080.cpp 확인함).
// 플레이스홀더로 255(미사용 핀 관례값) 전달.
BNO080 imu(255, 255, 255, 255);
