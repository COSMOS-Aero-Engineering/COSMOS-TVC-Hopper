...
3. **PR을 만들 때 리뷰어 `junwonkim07`을 직접 지정한다.** CODEOWNERS는 자동 리뷰 알림이 너무 많이
   생겨 삭제됐다(PR #12). 따라서 자동 지정된다고 생각하면 안 된다.
4. **로컬 검사와 CI가 모두 통과해야 승인 대상이다.** PR 전에 `python tasks.py check`를 실행하고,
   GitHub의 `sim — 환경 로딩 + PPO 스모크`가 초록불인지 확인한다.
5. **부장 승인 전에는 머지하지 않는다.** 머지 권한자는 김준원(`junwonkim07`)·김민찬(`MINBBBB1201`)
   두 명뿐이고, 본인이 올린 PR도 스스로 승인하거나 머지하지 않는다. 승인 뒤 새 커밋을 push했다면
   기존 리뷰가 오래된 것이 되므로 다시 확인·승인받는다.
6. **머지는 squash 방식만 쓴다.** 승인 후 권한자가 `gh pr merge --squash --delete-branch`로 머지한다.
...
- AI 에이전트는 브랜치 생성·검사·PR 생성까지 진행할 수 있지만, **머지는 매번 사람에게 별도로
  확인받는다.** 포괄적인 작업 지시는 머지 승인으로 보지 않는다.
...
# CI 통과 + 부장 Approve를 기다린다. 작성자가 직접 머지하지 않는다.
```

브랜치 이름은 `<커밋 태그>/<짧은 설명>` 형태다. 예: `fw/imu-test-spi-mode`,
`docs/timeline-overview`. 머지와 원격 브랜치 삭제는 승인받은 머지 권한자가 처리한다.
