...
- **김민찬(`MINBBBB1201`, 부장)·김준원(`junwonkim07`)은 CI만 통과하면 승인 없이 바로 머지 가능**
  (본인 PR도 스스로 머지 OK). **그 외 부원은 이 둘 중 한 명의 승인(Approve)을 받아야 머지 가능** —
  PR 리뷰어로 둘 중 한 명을 지정할 것. AI 에이전트도 이 등급을 그대로 따라 머지를 직접 실행할 수
  있다(단, 남의 PR은 승인 확인 후에만).
