...
// "드롭인 호환" 관계라 마운트 치수 대용 측정 유효) 캘리퍼스 실측. servo_body_w·servo_ear_to_shaft
// 2개는 아직 미측정 — 조사자료(kpower.com) 기반 플레이스홀더 유지, ⚠️ 실물 서보로 재대조 필요.
servo_body_l  = 23;    // 접선 방향 길이(몸통 긴변) — 실측 확정(2.3cm)
servo_body_w  = 12.2;  // 수직 방향 폭(두께) — ⚠️ 아직 미측정, 조사자료 기반 플레이스홀더
servo_body_h  = 28;    // 반경 방향 깊이(출력축 방향, 축 보스 제외 케이스만) — 실측 확정(2.8cm)
servo_ear_pitch = 33;         // 이어 홀 간격 — 실측 확정(3.3cm). 조사자료(28.0) 대비 5mm 큼, 마운트판 폭 재확인 필요
servo_ear_to_shaft = 9.9;     // 출력축 ↔ 가까운 쪽 이어 홀 거리 — ⚠️ 아직 미측정, 조사자료 기반 플레이스홀더
