...
teensy_l = 36; teensy_w = 18;            // Teensy 4.0 (핀헤더 포함 여유치) — 참고용
esc_l    = 36; esc_w    = 24; esc_t = 8; // ESC 보드 (브래킷 크기 산정용)
batt_l = 105; batt_w = 34; batt_h = 25;  // 4S 1300mAh 팩 공칭 치수
...
// MG90S 서보 (베인 직결 구동) — ⚠️ 실측 확인 대상
servo_body_l  = 22.8;  // 접선 방향 길이
servo_body_w  = 12.2;  // 수직 방향 폭
servo_body_h  = 22.5;  // 반경 방향 깊이(출력축 방향)
servo_ear_pitch = 28.0;      // 이어 홀 간격 — 제조사마다 27.5~28.5, 반드시 실측
servo_ear_to_shaft = 9.9;    // 출력축 ↔ 가까운 쪽 이어 홀 거리 — 반드시 실측
