...
> ⚠️ **이 절은 rev B 잔재였다 — 2026-09-18 폐기.** 아래 obs(12)/act(4) 스펙은 실제 구현
> (`sim/sim_stage1/hopper_aviary.py`, obs 6·act 5차원)과 이미 어긋나 있었다(`sim/README.md`가 이미
> "이전 계획의 12/4차원 문서와 다름, 이게 현재 정답"이라고 지적해둔 상태였음 — 이번에 원 소스를 고침).
> **정본은 이제 [`03-control-pipeline-design.md`](03-control-pipeline-design.md) §4**(State/Action
> Space, Stage별 구분 포함)·§5(Domain Randomization 갭 분석)·§2(3D 물리엔진 전환 로드맵)다. 아래는
> 옛 표를 삭제하고 남긴 자리 — 새로 뭔가 적지 않는다, 위 문서로 간다.
