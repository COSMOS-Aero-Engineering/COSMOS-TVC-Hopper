...
## 작업 규칙 — 브랜치·PR (2026-09-18 확정, 같은 날 머지권한·AI권한 수정)
...
3. **PR을 올린다.** 작성자가 김민찬·김준원 둘 중 하나가 아니면, 리뷰어로 그 둘 중 한 명을 지정한다.
   `gh pr create --base main --reviewer <담당자>` (웹에서 올릴 땐 우측 Reviewers에 지정)
4. **머지 권한 — 두 등급 (2026-09-18 수정).**
   - **김민찬(`MINBBBB1201`, 부장) · 김준원(`junwonkim07`)**: CI(`.github/workflows/ci.yml`)가
     초록불이면 **승인(Approve) 없이 바로 머지 가능.** 본인이 올린 PR도 스스로 머지해도 됨.
   - **그 외 전원**: CI 통과 + 위 두 명 중 한 명의 승인(Approve)이 있어야 머지 가능.
5. 머지: `gh pr merge --squash --delete-branch` (해당 등급 조건 충족 후)

**AI 에이전트에게 특히 해당되는 것 (2026-09-18 수정)**: PR 생성·CI 통과뿐 아니라 **머지도 직접
실행해도 된다** — 단 위 4번 등급을 그대로 따른다. 김민찬·김준원 본인이거나 그 둘을 대신해 진행 중인
작업이면 CI green 확인 후 바로 머지, 그 외 부원의 PR이면 **승인이 이미 기록된 걸 직접 확인한 뒤에만**
머지한다. 머지를 실행했으면 그 사실을 반드시 사람에게 알린다(조용히 넘어가지 않는다) — "다 해줘" 같은
포괄적 지시가 매번 재확인을 없앤다는 뜻이지, 투명성까지 없앤다는 뜻은 아니다.

*(최초 확정본은 "머지 가능자 2명, 그 2명도 승인 전엔 안 함, AI는 머지 자체를 안 함"이었는데, 같은 날
부장이 직접 위처럼 완화했다 — 실제 운영해보고 너무 빡빡하면 다시 조인다.)*

왜 원래 이렇게 바꿨나: 이전 규칙은 "작업 폴더가 팀별로 나뉘어 있으니 `main`에 직접 커밋"이었다. 충돌은
확실히 줄지만, 그 규칙의 진짜 비용은 충돌이 아니라 **깨진 코드가 곧바로 모두의 `git pull`에 실려 간다**는
점이다. 부원이 자기 작업을 시작하고 나서야 남의 실수를 발견하면 "내가 뭘 잘못했나"와 구분이 안 되고,
되돌릴 단위도 불분명해진다. PR은 (a) CI가 사람보다 먼저 깨진 걸 잡고, (b) 필요한 경우 머지권자가 한 번
보고, (c) 문제가 생겼을 때 되돌릴 단위가 PR 하나로 명확하다 — 이 셋을 동시에 준다.

**⚠️ 미해결**: 위 "브랜치·PR" 원칙 자체(부원도 전부 브랜치·PR 쓰나, AI만 해당인가)도 PR #1 본문에서
작성자가 직접 물어본 열린 질문이었다 — `README.md`는 일단 "전체 적용"으로 맞춰뒀다. junwonkim07의
정확한 직책(부장은 김민찬이니 다른 타이틀일 것)도 확인 필요 — 위에서 직책 없이 이름만 씀.
